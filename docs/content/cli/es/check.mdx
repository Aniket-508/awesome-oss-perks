...
| `-p, --program <slug>`      | Verificar contra un programa específico solamente (ej. `--program vercel`)         |
...
Elegibilidad en 48 programas — 12 elegibles, 8 requieren revisión, 28 no elegibles
  ...
  ✅  vercel                 Probablemente elegible — proyecto de código abierto
  ✅  netlify                Probablemente elegible — proyecto de código abierto
  ⚠️  github-copilot         Requiere revisión — revisión manual requerida
  ❌  aws-activate           Probablemente no elegible — requiere 1000 estrellas
```
...
Siempre verifica contra la página oficial del programa antes de aplicar.
